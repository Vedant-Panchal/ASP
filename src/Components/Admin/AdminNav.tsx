...
const AdminNav: React.FC<AdminNavProps> = ({ toggleDelete, handleUpload }) => {
  const context = useContext(UserContext);
  const navigate = useNavigate();

  if (!context) {
    throw new Error("AdminNav must be used within an AuthContextProvider");
  }
  ...
  const handleSignOut = async () => {
    const result = await Swal.fire({
      ...
      background: "#111827",
      color: "#FFFFF2",
      backdrop: "rgba(255, 255, 255, 0.4) left top no-repeat",
    });
      ...
      <div className="flex flex-wrap justify-between items-center relative">
        ...
        <div className="flex items-center lg:order-2">
          <button
            ...
            className="text-white bg-rose-700 hover:bg-rose-800 focus:ring-4 focus:ring-rose-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:bg-rose-600 dark:hover:bg-rose-700 focus:outline-none dark:focus:ring-rose-800"
          >
            Delete
          </button>
          <Link
