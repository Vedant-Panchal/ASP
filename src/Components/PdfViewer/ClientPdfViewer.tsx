...
interface TreeItem extends FileDoc {
  children?: TreeItem[];
}
...
const ClientPdfViewer: React.FC = () => {
  ...
  const tree: TreeItem[] = localStorage.getItem("tree") ? JSON.parse(localStorage.getItem("tree")!) : [];

  const searchFile = (data: TreeItem[], targetId: string): FileDoc | null => {
    for (const item of data) {
      if (item.url && item.id === targetId) {
        return item;
      }
      if (item.children?.length) {
        const found = searchFile(item.children, targetId);
  ...
  const fetchFileData = (targetId: string): FileDoc | null => {
    if (!tree?.length) {
      console.log("No data found in the tree");
      return null;
    }
    return searchFile(tree, targetId);
  };
        ...
        <nav className="bg-slate-100 px-4 dark:bg-darkNav dark:shadow-sm fixed left-0 right-0 top-0 z-[1000] shadow-lg rounded-sm">
          <div className="flex flex-wrap justify-between items-center relative">
            ...
            <div className="flex flex-row items-center justify-start w-fit lg:order-2">
              <button
                className={`${mode === "light" ? "bg-yellow-300" : "bg-darkElevate"} w-fit h-fit p-2 rounded-full transition-all duration-500 ease-in mr-2`}
                onClick={toggleMode}
              >
              ...
              >
                <img className="w-8 h-8 m-2 rounded-full" src={profilePicture} alt="user" />
              </button>
              ...
              >
                <div className="py-3 px-4">
                  ...
                  <Link className="block text-gray-900 truncate dark:text-white mt-5" to="/about-us">
                    About Us
                  </Link>
                  <Link className="block text-gray-900 truncate dark:text-white mt-5 mb-5" to="/contact-us">
                    Contact Us
                  </Link>
                  <Link
                    to="/sign-in"
