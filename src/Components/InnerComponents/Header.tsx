...
const MobileNavItem = ({ to, children }: NavLinkProps) => (
  <li>
    <NavLink
      to={to}
      className={({ isActive }) =>
...
function Header() {
  ...
  const toggleMenu = () => setHidden(prev => !prev);

  return (
    <header>
      <Link
        to="https://github.com/Mrunal-Shah7/ASP-Data/releases/download/v1.1.0/ASP-v1.1.0-arm64-v8a-release.apk"
      ...
      <nav className="fixed top-0 left-0 right-0 border-gray-200 shadow-sm lg:px-6 bg-Light10 px-4 dark:bg-darkNav dark:shadow-sm">
          ...
          >
            <ul className="flex flex-col font-medium lg:flex-row lg:space-x-8 lg:mt-0 lg:h-auto h-screen">
              <li className="ml-auto mb-5">
                <button
                  className="inline-flex items-center p-2 mt-1 ml-1 text-sm text-gray-500 rounded-lg lg:hidden hover:text-gray-900 hover:bg-slate-200/80 focus:bg-slate-200/80 dark:focus:bg-darkElevateHover dark:focus:ring-gray-700 dark:text-slate-200 dark:hover:bg-darkElevate dark:hover:text-slate-300 transition-all duration-200 ease-in"
                  hidden={hidden}
                  onClick={toggleMenu}
                >
                  <svg
              ...
              <MobileNavItem to="/about-us">About Us</MobileNavItem>
              <MobileNavItem to="/contact-us">Contact Us</MobileNavItem>
            </ul>
          </div>
        </div>
      </nav>
