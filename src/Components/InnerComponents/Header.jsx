...
return (
  ...
  const toggleMode = () => {
    ...
    <header>
    <nav className={` fixed top-0 left-0 right-0  border-gray-200  lg:px-6 bg-slate-100 px-4  dark:bg-darkNav dark:shadow-sm`}>
...
<div className="flex justify-between items-center w-max h-max">
      <button
        ...
        <div className={`justify-between items-center fixed top-0 left-0 transition-transform ease-in-out duration-200 bg-white border-r shadow-xl border-gray-200  dark:bg-darkNav dark:border-gray-700 w-1/2 lg:hidden ${hidden ? '-translate-x-full':'translate-x-0'}`} 
        ...
        >
          ...
          <ul className="flex flex-col font-medium lg:flex-row lg:space-x-8 lg:mt-0 lg:h-auto h-screen">
            ...
            <li>
              <NavLink
                to={"/"}
                className={({isActive})=> `block dark:text-white py-2 pr-4 pl-3 rounded lg:bg-transparent lg:p-0 ${isActive ? 'bg-primary-700 text-white lg:text-primary-700': 'text-gray-700'}`}
              >
                Home
              </NavLink>
            </li>
