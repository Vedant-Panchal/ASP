...
interface EditSemProps {
  index: string;
  semsData: SemData[];
  setSemsData: (data: SemData[]) => void;
}

const EditSem: React.FC<EditSemProps> = ({ index, semsData, setSemsData }) => {
  const [hidden, setHidden] = useState(true);
  const [sem, setSem] = useState<SemData | null>(null);
  const [semData, setSemData] = useState<SemData>({
    sem: "",
    sgpa: "",
    id: "",
  });
  ...
  const findItem = (id: string) => {
    const foundSem = semsData.find((sem) => sem.id === id);
    if (foundSem) {
      setSem(foundSem);
      setHidden(false);
    } else {
      console.error(`Sem with id ${id} not found.`);
    }
  };
